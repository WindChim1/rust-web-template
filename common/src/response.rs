...
pub struct ReopnseResult<T: Serialize> {
    pub code: u16,
    pub msg: String,
    // 业务数据
    pub data: T,
...
impl<T: Serialize> ReopnseResult<T> {
    /// 创建一个成功的响应，包含业务数据
    ...
    pub fn success_msg(msg: &str) -> ReopnseResult<()> {
        ReopnseResult {
            code: 200,
            msg: msg.to_string(),
            data: (),
    ...
    pub fn error(code: u16, msg: &str) -> ReopnseResult<()> {
        ReopnseResult {
            code,
            msg: msg.to_string(),
            data: (),
...
fn response_result_test() -> serde_json::Result<()> {
    ...
    let resutl = ReopnseResult::success(test_user);
